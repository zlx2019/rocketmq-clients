...
async fn main() {
    ...
    consumer_option.set_topics(vec!["example-golang-topic"]);
    consumer_option.set_consumer_group("example-golang-group");

    // set which rocketmq proxy to connect
    let mut client_option = ClientOption::default();
    client_option.set_access_url("118.145.206.25:8081");
    ...
    let receive_result = consumer
        .receive(
            "example-golang-topic".to_string(),
